...
?>
	...
	<div class="comment<?php if(!empty($comment->comment_parent)) { echo ' children'; } ?>" id="comment-<?php comment_ID() ?>">
		<div class="left">
		...
		<div class="comment_arrow"></div>
      	<div class="right">
			...
			<div class="comment_date">&nbsp;replied&nbsp;<?php echo pp_ago(strtotime($comment->comment_date)); ?></div>
			<br class="clear"/>
      		<?php ' '.comment_text() ?>
		...
		$return_html.= '<ul class="posts blog '.$bg_color.'_wrapper">';
			...
			foreach($posts as $post)
			{
				...
				if(has_post_thumbnail($post->ID, 'large'))
				{
				    ...
				    $small_image_thumb = $full_image_url = wp_get_attachment_image_src($photo->ID, 'thumbnail', true);
				}
				...
				{
					$return_html.= '<a href="'.get_permalink($post->ID).'"><img src="'.$small_image_thumb[0].'" width="50" height="50" alt="" class="frame" /></a>';
				}
				$return_html.= '<strong><a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a></strong><br/>';
				$return_html.= pp_substr(strip_tags(strip_shortcodes($post->post_content)), 80).'<br/><br/></li>';
...
function get_the_content_with_formatting ($more_link_text = '(more...)', $stripteaser = 0, $more_file = '') {
	...
	$content = '<div class="post_detail">'._substr(strip_tags(strip_shortcodes($content)), 600).'</div>';
	return $content;
}
